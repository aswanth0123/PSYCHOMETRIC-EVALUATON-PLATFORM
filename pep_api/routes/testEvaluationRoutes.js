...
const createTableQuery = `
    CREATE TABLE IF NOT EXISTS test_evaluation (
        ...
        TEST_ATTEND_ID INT NOT NULL,
        CREATED_AT TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        FOREIGN KEY (CANDIDATE_ID) REFERENCES candidates(ID) ON DELETE CASCADE,
        FOREIGN KEY (TEST_ID) REFERENCES test_details(TEST_ID) ON DELETE CASCADE,
        FOREIGN KEY (TEST_ATTEND_ID) REFERENCES test_attend(TEST_ATTEND_ID) ON DELETE CASCADE
    );
`;
...
router.get("/", (req, res) => {
    db.query(
        `SELECT te.TEST_EVALUATION_ID,c.ID, c.first_name, c.last_name, t.TEST_NAME, te.TEST_EVALUATION,te.TEST_ATTEND_ID,te.CREATED_AT
...
router.post("/", (req, res) => {
    const { CANDIDATE_ID, TEST_ID, TEST_EVALUATION,TEST_ATTEND_ID } = req.body;

    if (!CANDIDATE_ID || !TEST_ID || !TEST_EVALUATION) {
        return res.status(400).json({ error: "All fields are required" });
    }

    const insertQuery = "INSERT INTO test_evaluation (CANDIDATE_ID, TEST_ID, TEST_EVALUATION,TEST_ATTEND_ID) VALUES (?, ?, ?,?)";
    db.query(insertQuery, [CANDIDATE_ID, TEST_ID, TEST_EVALUATION,TEST_ATTEND_ID], (err, result) => {
        if (err) {
